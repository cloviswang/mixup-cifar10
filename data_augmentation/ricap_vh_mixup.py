...
class ricap_vh_mixup(object):
    ...
    def data(x, y, alpha=1.0, use_cuda=True):
        ...
        rhv_mixed_x = lam_mixup * x
        rW = random.randint(0, 32 - W)
        rH = random.randint(0, 32 - H)
        # print(f"rW={rW}, rH={rH}")
        rhv_mixed_x[:, :, 0:W, 0:H] = x[:, :, rW:(W + rW), rH:(H + rH)]
        rW = random.randint(0, W)
        rH = random.randint(0, H)
        # print(f"rW={rW}, rH={rH}")
        rhv_mixed_x[:, :, W:31, H:31] = x[index, :, (W - rW):(31 - rW), (H - rH):(31 - rH)]
        ...
        rhv_mixed_x[:, :, 0:W, H:31] = lam_mixup * x[:, :, rW_a:(W + rW_a), (H - rH_a):(31 - rH_a)] + \
